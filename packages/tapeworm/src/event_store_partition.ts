...
import type { DispatchHandler, IBaseEvent, ICommit, IPersistencePartition, ISnapshot, NodeCallback } from "./types";
...
interface IEventStorePartition {
  ...
  append(commits: ICommit | ICommit[], callback?: NodeCallback<unknown>): Promise<ICommit[]>;
  delete(streamId: string, deleteEvent?: Record<string, unknown>): Promise<void>;
  ...
  _queryStreamWithSnapshotFallback(streamId: string, callback?: NodeCallback<unknown>): Promise<unknown>;
  _truncateStreamFrom(streamId: string, commitSequence: number, callback?: NodeCallback<void>): Promise<void>;
  _applyCommitHeader(commitId: string, header: Record<string, unknown>, callback?: NodeCallback<ICommit>): Promise<ICommit>;
...
var EventStorePartition = function (
  ...
  var self = this;
  _.forEach(
    ["storeSnapshot", "loadSnapshot", "queryStream", "queryStreamWithSnapshot", "removeSnapshot", "getLatestCommit", "querySnapshotsByMaxDateTime"] as const,
    function (what: string) {
      // biome-ignore lint/complexity/noBannedTypes: dynamic method delegation requires generic function type
      var pp = self._persistencePartition as unknown as Record<string, Function>;
      if (pp[what]) {
        self[what] = function () {
        ...
        if (what === "queryStreamWithSnapshot") {
          // fallback function
          self[what] = function () {
            return self._queryStreamWithSnapshotFallback.apply(self._persistencePartition, arguments as unknown as [string, NodeCallback<unknown>?]);
          };
        }
      }
    }
...
EventStorePartition.prototype.openStream = function (
  ...
  var stream = new EventStream(this as unknown as ConstructorParameters<typeof EventStream>[0], streamId, writeOnly as boolean);
  return stream._prepareStream(callback);
};

EventStorePartition.prototype.append = function (this: IEventStorePartition, commits: ICommit | ICommit[], callback?: NodeCallback<unknown>) {
  //pre hooks
  var self = this;
  ...
  return Promise.each(commits, function (commit: ICommit) {
    return self._persistencePartition.append(commit, callback as NodeCallback<ICommit>).then(function (r: ICommit) {
      var done = function () {
 ...
 */
EventStorePartition.prototype.delete = function (this: IEventStorePartition, streamId: string, deleteEvent?: Record<string, unknown>) {
  var self = this;
  var event = Object.assign({}, deleteEvent, {
    id: uuid(),
    type: "$stream.deleted.event",
    payload: Object.assign({}, { dateOfDeletion: new Date() }, (deleteEvent && (deleteEvent.payload as Record<string, unknown>)) || {}),
  });
...
EventStorePartition.prototype._queryStream = function (this: IEventStorePartition, streamId: string, callback?: NodeCallback<ICommit[]>) {
  return this._persistencePartition.queryStream(streamId, callback);
};

EventStorePartition.prototype._queryAll = function (this: IEventStorePartition, callback?: NodeCallback<ICommit[]>) {
  return this._persistencePartition.queryAll(callback);
};

EventStorePartition.prototype._queryStreamWithSnapshotFallback = function (this: IPersistencePartition, streamId: string, callback?: NodeCallback<unknown>) {
  var self = this;
  return new Promise(function (resolve: (value: unknown) => void, reject: (reason: Error) => void) {
    self.loadSnapshot?.(streamId, function (err: Error | null, snapshot?: ISnapshot) {
      if (err) {
        reject(err);
      } else {
        var snapshotVersion = (snapshot && snapshot.version) || -1;
        self.queryStream(streamId, snapshotVersion, function (_err: Error | null, res?: ICommit[]) {
...
EventStorePartition.prototype._applyCommitHeader = function (
  ...
  return this._persistencePartition.applyCommitHeader?.(commitId, header, callback);
};
