...
const LOG = LoggerFactory.getLogger('tapeworm:event-store-partition');
...
EventStream.prototype.commit = function (commitId, callback) {
  ...
  var truncateStreamPromise = Promise.resolve();
  if (this._isDeleted) {
    LOG.info('Stream has been deleted. Commit to deleted stream. StreamId: %s', self._streamId);
    truncateStreamPromise = this._partition._truncateStreamFrom(self._streamId, -1)
  }
  ...
  if (!this.hasChanges()) {
    ...
    return truncateStreamPromise.then(() => {
        this._partition.append(commit, callback).then(function (commit) {
        //rebuild local state
